...
namespace DemoUpSchoolProject.Controllers
{
    public class MessageController : Controller
    {
        ...
        public ActionResult SendMessage(TblMessage p)
        {
            ...
            p.SenderMail = mail;
            p.SenderNameSurname = db.TblMember.Where(x => x.MemberMail == mail).Select(y => y.MemerName + " " + y.MemberSurname).FirstOrDefault();
            p.ReceiverNameSurname = db.TblMember.Where(x => x.MemberMail == p.ReceiverMail).Select(y => y.MemerName + " " + y.MemberSurname).FirstOrDefault();
            db.TblMessage.Add(p);
            db.SaveChanges();
